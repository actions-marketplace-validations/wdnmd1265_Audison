...
<p align="center">
  <strong>AI 写的代码，你的 AI 审查漏掉的 Bug，Audison 帮你找出来。</strong>
</p>
...
同一段 AI 生成的认证代码，三种审查工具：

| 发现了什么 | 标准 AI Review | CodeQL / SAST | Audison |
|---|---|---|---|
| Login 中的 SQL 注入 | ❌ 未发现 | ❌ 未发现 | 🔴 已发现 |
| 硬编码 JWT 密钥 | ⚠️ 警告 | ❌ 未发现 | 🔴 已发现 |
| 缺少频率限制 | ❌ 未发现 | ❌ 未发现 | 🟡 已发现 |
| CSRF Token 绕过 | ❌ 未发现 | ❌ 未发现 | 🔴 已发现 |

> 一个模型说"看起来没问题"。第二个模型不同意。Audison 让它们争论——你得到的是两者都发现不了的 Bug。
...
设置至少一个 API Key——或两个以实现跨提供商交叉验证（推荐）：
...
| 不确定性透明 | ✅ | ❌ | ❌ |
| 可验证证据链 | ✅ | ❌ | ❌ |
| 成本 | 免费软件；你只需支付自己的 API 费用 | $X/月订阅 | 免费（信任风险自担） |
...
核心洞察：单个模型无法发现自己的盲区。两个在不同数据上训练的模型，加上一个主动尝试破坏输出的对抗性对手，可以捕获到任何一个单独会遗漏的问题。
...
- [ ] **GitHub Action** → 自动 PR 审查评论，附 `--html` 报告链接
...
[Apache License 2.0](LICENSE) — 版权所有 2026 盛赟
